...
const login =catchAsync(async(req,res,next)=>{
    ...
    if(!cuenta || !password){
        return next(new AppError("Porfavor ingrese una cuenta y contraseña",400))
    }
...
const updatePassword = catchAsync(async(req,res,next)=>{
    ...
    user.confirmarPassword = confirmarPassword;
    await user.save();
    //4) Volver a iniciar sesion y enviar el JWT
    createSendToken(user,200,req,res);
    
});
module.exports = {registro, login,cerrarSesion ,protect, restrictTo, updatePassword };